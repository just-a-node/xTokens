...
contract PermitSignature {
  ...
  function getCompactPermitTransferSignature(
    ...
    bytes32 msgHash = keccak256(
      abi.encodePacked(
        ...
        domainSeparator,
        keccak256(
          abi.encode(_PERMIT_TRANSFER_FROM_TYPEHASH, tokenPermissions, address(this), permit.nonce, permit.deadline)
        )
      )
    );
  ...
  function _getCompactSignature(uint8 vRaw, bytes32 rRaw, bytes32 sRaw) internal pure returns (bytes32 r, bytes32 vs) {
  ...
  function getPermitBatchSignature(
    ...
    bytes32 msgHash = keccak256(
      abi.encodePacked(
        ...
        keccak256(
          abi.encode(
            _PERMIT_BATCH_TYPEHASH, keccak256(abi.encodePacked(permitHashes)), permit.spender, permit.sigDeadline
          )
        )
      )
    );
  ...
  function defaultERC20PermitAllowance(
    ...
    return
      IAllowanceTransfer.PermitSingle({details: details, spender: address(this), sigDeadline: block.timestamp + 100});
  }
  ...
  function defaultERC20PermitBatchAllowance(
    ...
    for (uint256 i = 0; i < tokens.length; ++i) {
      details[i] =
        IAllowanceTransfer.PermitDetails({token: tokens[i], amount: amount, expiration: expiration, nonce: nonce});
    }
  ...
  function defaultERC20PermitMultiple(
    ...
    return
      ISignatureTransfer.PermitBatchTransferFrom({permitted: permitted, nonce: nonce, deadline: block.timestamp + 100});
  }
}
